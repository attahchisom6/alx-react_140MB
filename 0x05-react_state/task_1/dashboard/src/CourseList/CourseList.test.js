...
describe('test the course list component', () => {
  ...
  it("verifies that Course list renders correctly if u pass an empty array", () => {
    ...
    expect(component.find("tbody").childAt(0).html()).toContain('<tr class="rowStyle_y7r86x"><td>No course available yet</td><td></td></tr>');
  });

  it("verifies that Course list renders correctly when no array was passed", () => {
    const component = shallow(<CourseList />);
    expect(component.find("tbody").childAt(0).html()).toEqual('<tr class=\"rowStyle_y7r86x\"><td>No course available yet</td><td></td></tr>');
});
  ...
  it("verifiea that CourseList renders correctly wen listCourse array is passed", () => {
    ...
    listCourses.forEach((course, index) => {
      expect(component.find("tbody").childAt(index).html()).toEqual(`<tr class="rowStyle_y7r86x"><td>${ course.name }</td><td>${ course.credit }</td></tr>`
      );
    });
  });
